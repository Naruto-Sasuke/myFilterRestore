function hist = getDataHist(flag,varargin)
hist=[];
%0为得到训练的直方图，1为得到测试的直方图
% 提取训练/测试数据的颜色直方图。
% varargin是传入要删去类型的正则表达式。
if flag == 0
    % 得到训练的颜色直方图。
...
%--------------------------------------------------------------------              
       ...
       ori=im2double(imread(oriStr{1}));
        ...
        mm = find(delIndices == 1);   %比如暖黄效果，假设得到的mm(1)是7，而对应的效果编号是6
        filterNum = numel(yy);
        for i = 1:filterNum          
...
elseif flag == 1
    for j=1:6
        ...
        if ischar(varargin{1}) 
           delFiltered = regexp({file_path_list.name},strcat('(',varargin{1},').jpg'),'tokens'); 
        end
       index = ~cellfun(@isempty,oriImg);     
       xx = find(index == 1);
       tt = oriImg{xx};
